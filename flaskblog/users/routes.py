...
from flaskblog.users.forms import (RegistrationForm, LoginForm, UpdateAccountForm,
                                   RequestResetForm, ResetPasswordForm, CreateCourseForm, UpdateDepartmentDetailsForm,
                                   UpdateDepartmentAccountDetailsForm, UpdateCourseForm, AcademicDetailsForm)
...
def register():
    ...
    if form.validate_on_submit():
        ...
        user = User(username=form.username.data, email=form.email.data, image_file='default.jpg',
                    date_of_birth=form.date_of_birth.data, contact_number=form.contact_number.data,
                    address=form.address.data, sex=form.sex.data, course_registered='SE Lab', course_taught='none',
                    user_type='student', password=hashed_password)
...
def register_faculty():
    ...
    if form.validate_on_submit():
        ...
        user = User(username=form.username.data, email=form.email.data, image_file='default.jpg',
                    date_of_birth=form.date_of_birth.data, contact_number=form.contact_number.data,
                    address=form.address.data, sex=form.sex.data, course_registered='none',
                    course_taught=form.course_taught.data, user_type='faculty', password=hashed_password)
        db.session.add(user)
        db.session.commit()
        flash('Your account has been created! You are now able to log in', 'success')
        return redirect(url_for('users.login'))
...
def register_admin():
    ...
    if form.validate_on_submit():
        hashed_password = bcrypt.generate_password_hash(form.password.data).decode('utf-8')
        user = User(username=form.username.data, email=form.email.data, image_file='default.jpg', date_of_birth='none',
                    contact_number='none', address='none', sex='none', course_registered='none', course_taught='none',
                    user_type='admin', password=hashed_password)
        db.session.add(user)
        db.session.commit()
        flash('Your account has been created! You are now able to log in', 'success')
        return redirect(url_for('users.login'))
...
def create_course():
    ...
    if form.validate_on_submit():
        course = Courses(name=form.name.data, OfferedCourses=form.OfferedCourses.data, CourseName=form.CourseName.data,
                         CourseDetails=form.CourseDetails.data)
